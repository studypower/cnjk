...
<script type="text/javascript">
	...
	<#if meetingRoomInfo.meetingDeviceList?exists>
	   <#list meetingRoomInfo.meetingDeviceList as meetingDevice>
			  test.push({"id":${meetingDevice.id?if_exists}});
	   </#list>
	</#if>
			...
			<div id="form_content">
				<div id="tabs">
					...
					<div id="project" style="overflow:auto;">
						<form id="staffInfoForm" name="staffInfoForm" action="${gzUrl('hysgl/xxwh/update.do')}" method="post">
							<table border="0" width="800" style="overflow: hidden;">
							    <input type="hidden" id="id" name="id" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.id?if_exists}"/>
								<tr>
									<td>
										<label for="name" class="edit_lbl">会议室名称:</label>
									</td>
									<td>
										<input type="text" id="name" name="name" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.name?if_exists}"/>
									</td>
                                  <td>
										<label for="maxCountUse" class="edit_lbl">人数:</label>
									</td>
									<td>
										<input type="text" id="maxCountUse" name="maxCountUse" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.max_count_use?if_exists}"/>
									</td>
								</tr>
								<tr>
									<td>
									...
									<td>
										<select name="active" id="active" class="text ui-widget-content" style="width:151px;">
							   		              <option value="1" <#if meetingRoomInfo.active?exists && meetingRoomInfo.active==1>selected</#if>>在用</option>
							   		              <option value="-1" <#if meetingRoomInfo.active?exists && meetingRoomInfo.active==-1>selected</#if>>不可用</option>
							           </select>
									</td>
								<#--
									<td>
									...
									<td>
										<input type="text" id="address" name="address" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.address?if_exists}"/>
									</td>-->
								</tr>
							<#--
								<tr>
									...
									<td colspan="3">
										<input type="text" id="startTime" name="startTime" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.start_time?if_exists}"/>-
										<input type="text" id="endTime" name="endTime" class="text ui-widget-content ui-corner-all" value="${meetingRoomInfo.end_time?if_exists}"/>
									</td>
								</tr>-->
								<tr>
									<td>
							   ...
							   <tr>
                                     <td><label class="edit_lbl" for="remark">备注:</label></td>
                                        <td colspan="3">
                                              <textarea id="remark" class="text ui-widget-content ui-corner-all" style="width: 494px;" name="remark" value="${meetingRoomInfo.remark?if_exists}">${meetingRoomInfo.remark?if_exists}</textarea>
                                        </td>
                                </tr>
								<tr>
									<td colspan="4">
