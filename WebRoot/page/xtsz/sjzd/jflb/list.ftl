...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        ...
	        colModel: [
	            ...
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	               ...
	               var html='';
            	   html += '<a href="${gzUrl('xtsz/sjzd/jflb/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
            	   if(rowObject['status']==0){
            		  html += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="${gzUrl('xtsz/sjzd/jflb/updateStatus.do')}?id=' + rowObject['id'] + '&status=1">启用</a>';
            	   }else if(rowObject['status']==1){
            	      html += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="${gzUrl('xtsz/sjzd/jflb/updateStatus.do')}?id=' + rowObject['id'] + '&status=0">禁用</a>';
            	   }	  
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/jflb/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/jflb/initAdd.do')}";
	    });
		$("#pageloading").hide();
	});
</script>
<body>
	...
	<div class="osx-container">
		<div class="osx-modal-header">
			...
			<div class="osx-modal-title">您现在所在的位置： 系统设置->数据字典->经费类别设置</div>
		</div>
