...
<script type="text/javascript">
	$(function(){
	        ...
	        colModel: [
	            ...
	            {name: "", width:100,sortable:false,align:"center",formatter:function(key,options,rowObject){
	               if(rowObject['isBlanklines']){
	            	...
	            	var html='<a href="${gzUrl('xtsz/sjzd/sb/initList.do?typeId=')}' + rowObject['id'] + '">点击查看</a>';
                	return html;
	            }},
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	...
	            	var html='';
	            	html += '<a href="${gzUrl('xtsz/sjzd/sblb/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/sblb/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/sblb/initAdd.do')}";
	    });
		$("#pageloading").hide();
	});
</script>
