...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        ...
	        colModel: [
	            ...
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	...
	            	var html='';
	            	html += '<a href="${gzUrl('xtsz/sjzd/whcdsz/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>&nbsp;&nbsp;';
	            	html += '<a href="${gzUrl('xtsz/sjzd/whcdsz/del.do')}?id=' + rowObject['id'] + '">删除</a>&nbsp;&nbsp;';
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/whcdsz/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/whcdsz/initAdd.do')}";
	    });
		$("#pageloading").hide();
	});
</script>
<body>
	...
	<div class="osx-container">
		<div class="osx-modal-header">
			...
			<div class="osx-modal-title">您现在所在的位置：系统设置->数据字典->文化程度设置</div>
		</div>
