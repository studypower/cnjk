...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        colNames: ["系统编号","设备名称","所属设备类别","品牌列表","操作"],
	        colModel: [
	            ...
	            {name: "type_name", width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	if(rowObject['isBlanklines'] || key==null){
	            		return "";
	            	}
	            	return key;
	            ...
	            {name: "", width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	...
	            	var html='<a href="${gzUrl('xtsz/sjzd/sbpp/initList.do?deviceId=')}' + rowObject['id'] + '">点击查看</a>';
                	return html;
	            }},
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	                if(rowObject['isBlanklines']){
	            	...
	            	var html='';
	            	html += '<a href="${gzUrl('xtsz/sjzd/sb/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/sb/list.do')}<#if typeId?exists>?typeId=${typeId}</#if>', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/sb/initAdd.do')}<#if typeId?exists>?typeId=${typeId}</#if>";
	    });
		$("#pageloading").hide();
	});
</script>
