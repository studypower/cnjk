...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        ...
	        colModel: [
	            ...
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	               	...
	               	var html='';
	               	if(rowObject['is_system']==0){
	               		html += '<a href="${gzUrl('xtsz/sjzd/gzzt/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
	        ...
	        url:'${gzUrl('xtsz/sjzd/gzzt/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/gzzt/initAdd.do')}";
	    });
		$("#pageloading").hide();
	...
	function del(id){
		$.get("${gzUrl('xtsz/sjzd/gzzt/del.do')}?id="+ id +"&timestamp=" + Date.parse(new Date()), null, callback);
	}
