...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        colNames: ["系统编号","设备品牌名称","所属设备","品牌型号列表","操作"],
	        colModel: [
	            ...
	            {name: "", width:100,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	...
	            	var html='<a href="${gzUrl('xtsz/sjzd/ppxh/initList.do?brandId=')}' + rowObject['id'] + '">点击查看</a>';
                	return html;
	            }},
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	...
	            	var html='';
	            	html += '<a href="${gzUrl('xtsz/sjzd/sbpp/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/sbpp/list.do')}<#if deviceId?exists>?deviceId=${deviceId}</#if>', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/sbpp/initAdd.do')}<#if deviceId?exists>?deviceId=${deviceId}</#if>";
	    });
		$("#pageloading").hide();
	});
</script>
