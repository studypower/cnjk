...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        ...
	        colModel: [
	            ...
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	               ...
	               var html='';
            	   html += '<a href="${gzUrl('xtsz/sjzd/jfdmc/initUpdate.do')}?id=' + rowObject['id'] + '">修改</a>';
            	   if(rowObject['status']==0){
            		  html += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="${gzUrl('xtsz/sjzd/jfdmc/updateStatus.do')}?id=' + rowObject['id'] + '&status=1">启用</a>';
            	   }else if(rowObject['status']==1){
            	      html += '&nbsp;&nbsp;&nbsp;&nbsp;<a href="${gzUrl('xtsz/sjzd/jfdmc/updateStatus.do')}?id=' + rowObject['id'] + '&status=0">禁用</a>';
            	   }	  
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/sjzd/jfdmc/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#create").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/sjzd/jfdmc/initAdd.do')}";
	    });
		$("#pageloading").hide();
	});
</script>
