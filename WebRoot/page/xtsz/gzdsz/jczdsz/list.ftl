...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        ...
	        colModel: [
	            ...
	            {name: "",width:250,sortable:false,align:"center",formatter:function(key,options,rowObject){
	              	...
	              	var html='';
	              	if(rowObject['system']==0){
		              	if(rowObject['status']==0){
		            		html += '<a href="${gzUrl('xtsz/gzdsz/jczdsz/status.do?status=1&id=')}'+rowObject['id']+'" title="启用">启用</a>&nbsp;&nbsp;';
		            	} else if(rowObject['status']==1){
		            		html += '<a href="${gzUrl('xtsz/gzdsz/jczdsz/status.do?status=0&id=')}'+rowObject['id']+'" title="禁用">禁用</a>&nbsp;&nbsp;';
		            	}
	            	}
	            	html += '<a href="${gzUrl('xtsz/gzdsz/jczdsz/info.do?id=')}'+rowObject['id']+'" title="详情">详情</a>&nbsp;&nbsp;';
	            	if(rowObject['system']==0){
	            		html += '<a href="${gzUrl('xtsz/gzdsz/jczdsz/initUpdate.do?id=')}'+rowObject['id']+'" title="修改">修改</a>&nbsp;&nbsp;';
	            	}
	            	if(rowObject['system']==0){
	        ...
	        url:'${gzUrl('xtsz/gzdsz/jczdsz/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#addBtn").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/gzdsz/jczdsz/initAdd.do?tableId=0')}";
	    });
		$("#pageloading").hide();
	});
	...
	function deleteColumn(id) {
		...
		if(goto){
			window.location.href="${gzUrl('xtsz/gzdsz/jczdsz/delete.do?id=')}" + id;
		}
	}
</script>
<body>
