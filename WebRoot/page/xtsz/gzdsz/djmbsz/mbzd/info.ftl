<#include "../../../../head.ftl" />
<script type="text/javascript">
	$(function(){
		$("#tabs" ).tabs();
		$("#tabs").height($(window).height()-35);
		$("#tabs > div").height($(window).height()-102);
		$(window).resize(function(){
			var height=$(window).height();
			$("#tabs").height(height-35);
			$("#tabs > div").height(height-102);
		});
		$("#saveBtn").button().click(function( event ) {
	    	event.preventDefault();
	    	$("#form1").submit();
	    });
	    
	    $("#columnType").combobox({
		});
	    $("#pageloading").hide();
	});
</script>
<body>
	<div id="pageloading"></div> 
	<div class="osx-container">
		<div class="osx-modal-header">
			<div class="osx-modal-icon"></div>
			<div class="osx-modal-title">您现在所在的位置：系统设置->工资单设置->单据模版设置->模板字段设置</div>
		</div>
		<div class="osx-modal-close" style="display: block;cursor: pointer;">
			<div class="osx-modal-icon-back">
				<div class="osx-modal-title" style="text-align: right;line-height: 13px;">
				  <a href="javascript:history.go(-1);"><img src="${gzUrl('images/a_back.png')}" width="14" height="14" />返回</a>
				</div>  
			</div>
		</div>
		<div class="osx-modal-content" id="osx-modal-content">
			<div id="form_content">
				<div id="tabs">
					<ul>
						<li><a href="#main_content" style="font-size: ;font-family: 'yakov_wryh';">详情</a></li>
					</ul>
					<div id="main_content" style="overflow:auto;">
						<form id="form1" name="form1" action="${gzUrl('xtsz/gzdsz/jczdsz/add.do')}" method="post">
							<table border="0" width="800" style="overflow: hidden;">
								<tr>
									<td width="70px">
										<label for="name" class="edit_lbl">字段名称:</label>
									</td>
									<td>
										<input type="text" id="name" name="name" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.name}" readonly/>
									</td>
									<td width="70px">
										<label for="projectSourceId" class="edit_lbl">字段别名:</label>
									</td>
									<td>
										<input type="text" id="columnName" name="columnName" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.column_name}" readonly/>
									</td>
								</tr>
								<tr>
									<td>
										<label for="columnType" class="edit_lbl">字段类型:</label>
									</td>
									<td>
						                <input type="text" id="columnName" name="columnName" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.column_type}" readonly/>
									</td>
									<td>
										<label for="columnType" class="edit_lbl">字段长度:</label>
									</td>
									<td>
						                <input type="text" id="columnName" name="columnName" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.column_length}" readonly/>
									</td>
								</tr>
								<tr>
									<td>
										<label for="columnRequired" class="edit_lbl">可否为空:</label>
									</td>
									<td>
										<#if payrollColumnInfo.column_required==0>
										<input type="radio"  name="columnRequired" value="0" checked/><span class="edit_lbl">是</span>
										<#elseif payrollColumnInfo.column_required==1>
										<input type="radio"  name="columnRequired" value="1" checked/><span class="edit_lbl">否</span>
										</#if>
									</td>
									<td>
										<label for="columnDefault" class="edit_lbl">默认值:</label>
									</td>
									<td>
										<input type="text" id="columnDefault" name="columnDefault" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.column_default}" readonly/>
									</td>
								</tr>
								<tr>
									<td>
										<label for="indexNum" class="edit_lbl">显示顺序:</label>
									</td>
									<td colspan="3">
										<input type="text" id="indexNum" name="indexNum" class="text ui-widget-content ui-corner-all" value="${payrollColumnInfo.index_num}" readonly/>
									</td>
								</tr>
								<tr>
									<td>
										<label for="remark" class="edit_lbl">字段描述:</label>
									</td>
									<td colspan="3">
										<textarea class="text ui-widget-content ui-corner-all" name="remark" id="remark" style="width: 494px;" readonly>${payrollColumnInfo.remark?if_exists}</textarea>
									</td>
								</tr>
							</table>
						</form>
					</div>
				</div>
			</div>
		</div>
	</div>
</body>
<#include "../../../../foot.ftl" />
