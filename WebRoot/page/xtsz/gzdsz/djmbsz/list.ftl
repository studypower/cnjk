...
<script type="text/javascript">
	$(function(){
		...
		$("#dataGrid").jqGrid({
	        colNames: ["系统编号","模板名称","模板别名","模板版本号","状态","创建时间","操作"],
	        colModel: [
	            ...
	            {name: "table_name",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	if(rowObject['isBlanklines'] || key==null){
	            		return "";
	            	}
	            	return key;
	            }},
	            {name: "version",width:100,sortable:false,align:"center",formatter:function(key,options,rowObject){
	            	if(rowObject['isBlanklines'] || key==null){
	            		return "";
	            	}
	            	return key;
	            ...
	            {name: "",width:200,sortable:false,align:"center",formatter:function(key,options,rowObject){
	                if(rowObject['isBlanklines']){
	              	...
	              	if(rowObject['status']==0){
	            		html += '<a href="${gzUrl('xtsz/gzdsz/djmbsz/status.do?status=1&id=')}'+rowObject['id']+'" title="启用">启用</a>&nbsp;&nbsp;';
	            	} else if(rowObject['status']==1){
	            		html += '<a href="${gzUrl('xtsz/gzdsz/djmbsz/status.do?status=0&id=')}'+rowObject['id']+'" title="禁用">禁用</a>&nbsp;&nbsp;';
	            	}
	            	html += '<a href="${gzUrl('xtsz/gzdsz/djmbsz/initUpdate.do?id=')}'+rowObject['id']+'" title="名称修改">名称修改</a>&nbsp;&nbsp;';
	            	if(rowObject['build']==0){
	            		html += '<a href="${gzUrl('xtsz/gzdsz/djmbsz/build.do?id=')}'+rowObject['id']+'" title="构建">构建</a>&nbsp;&nbsp;';
	            	}
	            	html += '<a href="${gzUrl('xtsz/gzdsz/djmbsz/mbzd/initList.do?tableId=')}'+rowObject['id']+'" title="字段列表">字段列表</a>';
                	return html;
	            }}
	        ],
	        datatype: "json", //将这里改为使用JSON数据  
	        url:'${gzUrl('xtsz/gzdsz/djmbsz/list.do')}', //这是Action的请求地址  
	        mtype: 'POST',
	        pager: $("#pager"),
	        rowNum:20,
	        shrinkToFit:false,
	    ...
	    $("#addBtn").button().click(function( event ) {
	    	...
	    	window.location.href="${gzUrl('xtsz/gzdsz/djmbsz/initAdd.do')}";
	    });
		$("#pageloading").hide();
	});
</script>
