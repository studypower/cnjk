			...
			<div id="form_content">
				<div id="tabs">
					...
					<div id="add" style="overflow:auto;">
						<form id="form1" name="form1" action="${gzUrl('xtsz/bmgl/zw/add.do')}" method="post">
									...
									<td colspan="3">
										<select id="positionId" name="positionId" class="select01">
					                		...
					                		<#list positionInfoList as positionInfo>
					                			...
					                			<#if num == departmentPositionList.size()>
					                				<option value="${positionInfo.id}" <#if form.positionId?exists && positionInfo.id==form.positionId>selected</#if>>${positionInfo.name}</option>
					                			</#if>
					                		</#list>
					                	</select>
									</td>
