...
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getChecked: function (){
            ...
            var nodes = [];
            $(".l-checkbox-checked,.l-checkbox-incomplete", g.tree).parent().parent("li").each(function (){
        ...
        loadData: function (node, url, param){
            var g = this, p = this.options;
            g.loading.show();
            var ajaxtype = p.method ? p.method : "get";
            param=$.extend(param || {},p.params||{});
            param = param || [];
            var async=(p.async==false?false:true);
            //请求服务器
            $.ajax({
                type: ajaxtype,
                url: url,
        ...
        getNodeDom: function (nodeParm){
            ...
            }else if (typeof (nodeParm) == "object" && 'treedataindex' in nodeParm){
        ...
        selectNode: function (selectNodeParm){
            ...
            if (typeof (selectNodeParm) == "function"){
                ...
                var treeitembody = $(">div:first", treeitem);
                if (p.checkbox){
                	$(".l-checkbox", treeitembody).removeClass("l-checkbox-unchecked");
                	$(".l-checkbox", treeitembody).addClass("l-checkbox-checked");
                }else{
        ...
        setWidth:function(width){
        	var g = this, p = this.options;
        	p.nodeWidth+=width-g.tree.width();
        	g._upadteTreeWidth();
        },
        ...
        _setTreeEven: function (){
            ...
            g.tree.click(function (e){ 
                ...
                if ($(obj).hasClass("l-checkbox")){
                    if (p.autoCheckboxEven){//状态：未选中
                        ...
                        if ($(obj).hasClass("l-checkbox-unchecked")){
                            ...
                            data=[{ data: treenodedata, target: treeitem[0] }, true];
                            //g.trigger('check', [{ data: treenodedata, target: treeitem[0] }, true]);
                        }else if ($(obj).hasClass("l-checkbox-checked")){//状态：选中
                            ...
                            data=[{ data: treenodedata, target: treeitem[0] }, false];
                            //g.trigger('check', [{ data: treenodedata, target: treeitem[0] }, false]);
                        }else if ($(obj).hasClass("l-checkbox-incomplete")){ //状态：未完全选中
                ...
                }else if (treeitembtn.hasClass("l-expandable-open") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn)){ //状态：已经张开
                ...
                }else if (treeitembtn.hasClass("l-expandable-close") && (!p.btnClickToToggleOnly || clickOnTreeItemBtn)){//状态：没有张开
