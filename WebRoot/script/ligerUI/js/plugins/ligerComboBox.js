...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setValue: function (value)
        {
            ...
            if((text== undefined ||text=="")&&!p.allowBlank){
            	text=value;
            }
            if (p.tree)
            {
            ...
            else if(g.gridManager){
            	var key=[],text=[];
            	$(value).each(function(index,data){
            		key.push(data[p.valueField]);
            		text.push(data[p.textField]);
            	});
            	g._changeValue(key.join(p.split), text.join(p.split));
            }
        ...
        bulidContent: function ()
        {
            ...
            {
            	var params=p.params||{};
            	$.extend(params,{"_time":new Date().getTime()});
                $.ajax({
                    type: 'post',
                    url: p.url,
                    cache: false,
        ...
        loadData:function(url,params,async){
        	var g = this, p = this.options;
        	params=params||{};
        	$.extend(params,{"_time":new Date().getTime()});
            $.ajax({
        ...
        setData: function (data)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                {
                	var tr =null;
                	if(p["initValue"]&&val==p["initValue"]){
                		tr = $("<tr value='" + val + "'><td style='width:18px;'  index='" + i + "' value='" + val + "' text='" + txt + "' ><input type='checkbox' checked='checked'/></td></tr>");
                	}else{
                		tr = $("<tr value='" + val + "'><td style='width:18px;'  index='" + i + "' value='" + val + "' text='" + txt + "' ><input type='checkbox' /></td></tr>");
                	}
        ...
        setGrid: function (grid)
        {
            ...
            var selecteds = p.selecteds||[];
            grid.columnWidth = grid.columnWidth || 120;
            grid.width = "100%";
            grid.height = "100%";
            grid.heightDiff = -2;
            grid.InWindow = false;
            grid.isChecked=function(rowdata){
            	return findSelected(rowdata);
            };
            g.gridManager = g.grid.ligerGrid(grid);
            p.hideOnLoseFocus =true;
            var onGridSelect = function () { 
                var value = [], text = [];
            ...
            var removeSelected = function (rowdata) {
            ...
            var addSelected = function (rowdata) {
                for (var i = selecteds.length - 1; i >= 0; i--) {
                    if (selecteds[i][p.valueField] == rowdata[p.valueField]) {
                        return;
                    }
            ...
            var findSelected = function(rowdata){
            	for (var i = selecteds.length - 1; i >= 0; i--) {
                    if (selecteds[i][p.valueField] == rowdata[p.valueField]) {
                        return true;
                    }
            ...
            if (grid.checkbox != false)
            {
                ...
                g.gridManager.bind('CheckAllRow', function (checked) {
                    $(g.gridManager.rows).each(function (rowindex,rowdata) {
                        onCheckRow(checked, rowdata);
                    });
                    onGridSelect();
                });
                g.gridManager.bind('CheckRow', function (checked, rowdata) {
                    onCheckRow(checked, rowdata);
                    onGridSelect();
                });
            }
            ...
            {
                g.gridManager.bind('SelectRow', function (rowdata) {
                    selecteds = [rowdata]; 
                    onGridSelect();
                });
                g.gridManager.bind('UnSelectRow', function () {
                    selecteds = [];
                    onGridSelect();
                });
            }
