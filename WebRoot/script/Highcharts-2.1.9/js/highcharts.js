...
(function () {
	...
	function Nd(a, b) {
		function c(p) {
			...
			function x(k, n, t, r) {
				...
				this.x = t;
				this.stack = r;
				this.alignOptions = {align:k.align || (va ? n ? "left" : "right" : "center"), verticalAlign:k.verticalAlign || (va ? "middle" : n ? "bottom" : "top"), y:A(k.y, va ? 4 : n ? 14 : -6), x:A(k.x, va ? n ? -6 : 6 : 0)};
			...
			function da() {
				...
				k = o.maxZoom || (xa && !K(o.min) && !K(o.max) ? tb(l.smallestInterval * 5, wa - pa) : null);
				ya = R ? Da : Aa;
				if (Ub) {
				...
				Ta = ia === ra ? 1 : Ub && !J && E === t.options.tickPixelInterval ? t.tickInterval : A(J, Ya ? 1 : (ra - ia) * E / ya);
				...
				if (D) {
					Ba = [];
					J = Xa.global.useUTC;
					var Q = 1000 / ub, fa = 60000 / ub, ka = 3600000 / ub;
					E = 86400000 / ub;
					k = 604800000 / ub;
					r = 2592000000 / ub;
					var $ = 31556952000 / ub, F = [["second", Q, [1, 2, 5, 10, 15, 30]], ["minute", fa, [1, 2, 5, 10, 15, 30]], ["hour", ka, [1, 2, 3, 4, 6, 8, 12]], ["day", E, [1, 2]], ["week", k, [1, 2]], ["month", r, [1, 2, 3, 4, 6]], ["year", $, null]], O = F[6], S = O[1], aa = O[2];
			...
			function eb() {
				...
				if (r) {
					...
					if (La) {
						...
						La = ga.path(J).attr({stroke:o.lineColor, "stroke-width":r, zIndex:7}).add();
					}
				}
				if (ba) {
					J = R ? X : ea;
					...
					ba[ba.isNew ? "attr" : "animate"]({x:R ? J : r + (Oa ? Da : 0) + la + (k.x || 0), y:R ? r - (Oa ? Aa : 0) + la : J + (k.y || 0)});
					ba.isNew = false;
				}
				if (n && n.enabled) {
					var fa, ka;
					n = I.stackTotalGroup;
					if (!n) {
						I.stackTotalGroup = n = ga.g("stack-labels").attr({visibility:Ab, zIndex:6}).translate(X, ea).add();
					}
					for (fa in s) {
						k = s[fa];
						for (ka in k) {
			...
			var xa = p.isX, Oa = p.opposite, R = va ? !xa : xa, L = R ? Oa ? 0 : 2 : Oa ? 1 : 3, s = {}, o = Ca(xa ? Yc : od, [ge, he, Td, ie][L], p), I = this, ba, B = o.type, D = B === "datetime", ca = B === "logarithmic", la = o.offset || 0, qa = xa ? "x" : "y", ya, fb, Hb, cc = R ? X : vb, G, ha, na, Ra, La, pa, wa, zb, Vb, Gb, ra = null, ia = null, Dc, Sd, Qd = o.minPadding, Rd = o.maxPadding, Ub = K(o.linkedTo), Od, Pd, Vc;
			B = o.events;
			var pd, Pb = [], Ta, Wc, Fb, Ba, mb = {}, Wb = {}, dc = {}, qc, rc, nd, Xc, Ya = o.categories, je = o.labels.formatter || function () {
				var k = this.value;
				return Xc ? Zc(Xc, k) : Ta % 1000000 === 0 ? k / 1000000 + "M" : Ta % 1000 === 0 ? k / 1000 + "k" : !Ya && k >= 1000 ? Ed(k, 0) : k;
			}, $c = R && o.labels.staggerLines, ec = o.reversed, fc = Ya && o.tickmarkPlacement === "between" ? 0.5 : 0;
			q.prototype = {addLabel:function () {
				var k = this.pos, n = o.labels, t = !(k === ia && !A(o.showFirstLabel, 1) || k === ra && !A(o.showLastLabel, 0)), r = Ya && R && Ya.length && !n.step && !n.staggerLines && !n.rotation && Da / Ya.length || !R && Da / 2, J = Ya && K(Ya[k]) ? Ya[k] : k, E = this.label;
				...
				if (S) {
					...
					if (F) {
						...
						this.mark = ga.path(t).attr({stroke:aa, "stroke-width":S}).add(na);
					}
				}
				if (r && !isNaN(Qb)) {
			...
			w.prototype = {render:function () {
				...
				if (F) {
					if (O) {
						F.animate({d:O}, null, F.onGetPath);
					} else {
						F.hide();
				...
				if (t && K(t.text) && O && O.length && Da > 0 && Aa > 0) {
					t = Ca({align:R && ka && "center", x:R ? !ka && 4 : 10, verticalAlign:!R && ka && "middle", y:R ? ka ? 16 : 10 : ka ? 6 : -4, rotation:R && !ka && 90}, t);
					if (!r) {
						k.label = r = ga.text(t.text, 0, 0).attr({align:t.textAlign || t.align, rotation:t.rotation, zIndex:aa}).css(t.style).add();
					}
			...
			x.prototype = {destroy:function () {
				...
				if (this.label) {
					...
					this.label = l.renderer.text(n, 0, 0).css(this.options.style).attr({align:this.textAlign, rotation:this.options.rotation, visibility:ob}).add(k);
				}
				...
				t = {x:va ? t ? r : r - J : E, y:va ? Q - E - n : t ? Q - r - J : Q - r, width:va ? J : n, height:va ? n : J};
			...
			sa(I, {addPlotBand:za, addPlotLine:za, adjustTickAmount:function () {
				...
				if (k || Ub) {
					u(Ba, function (fa) {
						...
						if (L === 0 || L === 2 || {1:"left", 3:"right"}[L] === J.align) {
				...
				if (r && r.text) {
					if (!ba) {
						ba = I.axisTitle = ga.text(r.text, 0, 0, r.useHTML).attr({zIndex:7, rotation:r.rotation || 0, align:r.textAlign || {low:"left", middle:"center", high:"right"}[r.align]}).css(r.style).add();
						ba.isNew = true;
					}
		...
		function e(p) {
			function q() {
				...
				var la = Sb(ca) || D, qa;
				qa = la ? ["<span style=\"font-size: 10px\">" + (D ? Zc("%A, %b %e, %Y", ca) : ca) + "</span>"] : [];
				u(B, function (ya) {
			...
			Ma.padding = 0;
			var o = ga.g("tooltip").attr({zIndex:8}).add(), I = ga.rect(ab, ab, 0, 0, p.borderRadius, N).attr({fill:p.backgroundColor, "stroke-width":N}).add(o).shadow(p.shadow), ba = ga.text("", eb + ab, ja(Ma.fontSize) + eb + ab, p.useHTML).attr({zIndex:1}).css(Ma).add(o);
			o.hide();
		...
		function f(p) {
			...
			function x() {
				var L = l.hoverSeries, s = l.hoverPoint;
				s && s.onMouseOut();
				L && L.onMouseOut();
				hc && hc.hide();
				sd = null;
			...
			bd = function () {
				if (Ec) {
					...
					l.trackerGroup = Ec = ga.g("tracker").attr({zIndex:9}).add();
				}
			};
			bd();
			if (p.enabled) {
						...
						if (ud && s.type !== "touchstart") {
							...
							if (Ma > 10) {
								if (sc && (ab || xa) && gc(ta - X, da - ea)) {
									za || (za = ga.rect(X, ea, Oa ? 1 : Da, R ? 1 : Aa, 0).attr({fill:z.selectionMarkerFill || "rgba(69,114,167,0.25)", zIndex:7}).add());
								}
								if (za && Oa) {
									o = o - ta;
							...
							if (!ba) {
								...
								if (o && o.tracker) {
									(s = o.tooltipPoints[qa]) && s !== I && s.onMouseOver();
								}
							}
						}
						return ba || !sc;
				...
				oa.onclick = function (s) {
					...
					if (!Ma) {
						if (o && Ga(s.target, "isTracker")) {
							...
							sa(o, {pageX:Jb.left + X + (va ? Da - ba : I), pageY:Jb.top + ea + (va ? Aa - I : ba)});
		...
		function g(p) {
			var q = p.type || z.type || z.defaultSeriesType, w = wb[q], x = l.hasRendered;
			if (x) {
				if (va && q === "column") {
					w = wb.bar;
				} else {
		...
		function m(p, q) {
			...
			u([["title", p, uc], ["subtitle", q, Ic]], function (w) {
				...
				if (w && w.text && !V) {
					l[x] = ga.text(w.text, 0, 0, w.useHTML).attr({align:w.align, "class":"highcharts-" + x, zIndex:1}).css(w.style).add().align(w, false, Rb);
				}
			});
		}
		function v() {
			...
			l.container = oa = hb(Tb, {className:"highcharts-container" + (z.className ? " " + z.className : ""), id:Wd}, sa({position:Xd, overflow:ob, width:$a + Ua, height:cb + Ua, textAlign:"left"}, z.style), Zb || qb);
		...
		function Y() {
			...
			if (!l.seriesGroup) {
				l.seriesGroup = ga.g("series-group").attr({zIndex:3}).add();
			}
			u(Ha, function (x) {
				x.translate();
				x.setTooltipPoints();
			...
			p.items && u(p.items, function () {
				var x = sa(p.style, this.style), V = ja(x.left) + X, N = ja(x.top) + ea + 12;
				delete x.left;
				delete x.top;
		...
		M = Nb(M) ? M : [M, M, M, M];
		var y = A(z.marginTop, M[0]), C = A(z.marginRight, M[1]), Z = A(z.marginBottom, M[2]), Sa = A(z.marginLeft, M[3]), Na = z.spacingTop, Ea = z.spacingRight, gb = z.spacingBottom, Lb = z.spacingLeft, Rb, uc, Ic, ea, Ib, vb, X, Xb, qb, Zb, oa, Wd, dd, Jc, $a, cb, qd, ad, wc, Kc, Ad, xc, l = this, ke = (M = z.events) && !!M.click, Bd, gc, hc, ud, jc, Zd, Cd, Aa, Da, Yb, Ec, bd, Fc, $b, ac, Jb, sc = z.showAxes, cd = 0, Va = [], Ob, Ha = [], va, ga, rd, Vd, sd, vd, Gc, yd, zd, xd, td, $d, le = function () {
			function p(G, ha) {
			...
			function x(G) {
				...
				if (!pa) {
					...
					G.legendItem = pa = ga.text(N.labelFormatter.call(G), 0, 0).css(G.visible ? ab : Oa).on("mouseover", function () {
						G.setState(Bb);
						pa.css(xa);
					...
					if (!La && wa && wa.lineWidth) {
						...
						G.legendLine = ga.path([Za, -da - Ma, 0, Ka, -Ma, 0]).attr(Vb).add(qa);
					}
					if (La) {
						ha = ga.rect(na = -da - Ma, Ra = -11, da, 12, 2).attr({zIndex:3}).add(qa);
					} else {
						if (wa && wa.marker && wa.marker.enabled) {
							ha = ga.symbol(G.symbol, na = -da / 2 - Ma, Ra = -4, wa.marker.radius).attr({zIndex:3}).add(qa);
						}
					}
					if (ha) {
						ha.xOff = na + zb % 2 / 2;
					...
					if (wa && wa.showCheckbox) {
						...
						Qa(G.checkbox, "click", function (Gb) {
							Pa(G, "checkboxClick", {checked:Gb.target.checked}, function () {
								G.select();
							});
						});
					}
			...
			function V() {
				...
				za.length && qa.align(sa(N, {width:$b, height:ac}), true, Rb);
				cd || w();
			}
		...
		vd = function () {
			...
			if (p || q) {
				if (wc) {
					...
					wc = ga.rect(V / 2, V / 2, $a - V, cb - V, z.borderRadius, p).attr({stroke:z.borderColor, "stroke-width":p, fill:q || jb}).add().shadow(z.shadow);
				}
			}
			if (w) {
				if (Kc) {
					...
					Kc = ga.rect(X, ea, Da, Aa, 0).attr({fill:w}).add().shadow(z.plotShadow);
				}
			}
			if (x) {
				if (Ad) {
			...
			if (z.plotBorderWidth) {
				if (xc) {
					...
					xc = ga.rect(X, ea, Da, Aa, 0, z.plotBorderWidth).attr({stroke:z.plotBorderColor, "stroke-width":z.plotBorderWidth, zIndex:4}).add();
				}
			}
			l.isDirtyBox = false;
		};
	...
	Zc = function (a, b, c) {
		...
		b = {a:v[g].substr(0, 3), A:v[g], d:d(h), e:h, b:m.shortMonths[i], B:m.months[i], m:d(i + 1), y:j.toString().substr(2, 2), Y:j, H:d(f), I:d(f % 12 || 12), l:f % 12 || 12, M:d(b[id]()), p:f < 12 ? "AM" : "PM", P:f < 12 ? "am" : "pm", S:d(b.getSeconds())};
		for (e in b) {
			a = a.replace("%" + e, b[e]);
		}
	...
	if (!ma && db.jQuery) {
		...
		u(["cur", "_default", "width", "height"], function (a, b) {
			var c = b ? ee : de.prototype, d = c[a], e;
			if (d) {
				c[a] = function (f) {
					f = b ? f : this;
					e = f.elem;
	...
	xb.bar = Ca(xb.column, {dataLabels:{align:"left", x:5, y:0}});
	...
	Uc.prototype = {init:function (a, b) {
		...
		var b = this.renderer, c = a || b, d = c.element || b.box, e = d.childNodes, f = this.element, g = Ga(f, "zIndex");
	...
	ed.prototype = {Element:Uc, init:function (a, b, c, d) {
		...
		if (i) {
			if (!j) {
				j = a.htmlNode = hb("span", null, sa(h, {position:ic, top:0, left:0}), this.box.parentNode);
			}
		...
		this.width = a;
		this.height = b;
		for (this.boxWrapper[A(c, true) ? "animate" : "attr"]({width:a, height:b}); e--; ) {
			d[e].align();
		}
	}, g:function (a) {
		...
		c = 0.707 * c;
		return [Za, a - c, b - c, Ka, a + c, b - c, a + c, b + c, a - c, b + c, "Z"];
		...
		var e = d.start, f = d.end - 0.000001, g = d.innerR, h = rb(e), i = Cb(e), j = rb(f);
		f = Cb(f);
		d = d.end - e < kc ? 0 : 1;
		...
		if (a && a.linearGradient) {
			...
			u(a.stops, function (j) {
				...
				j = f.createElement("stop").attr({offset:j[0], "stop-color":h, "stop-opacity":i}).add(g);
				g.stops.push(j);
			});
	...
	if (!Lc) {
		Db = yb(Uc, {init:function (a, b) {
				...
				for (c in a) {
					...
					if (j && /^(x|y|r|start|end|width|height|innerR)/.test(c)) {
						...
						if (c === "d") {
							...
							e = d.length;
							for (m = []; e--; ) {
								m[e] = lc(d[e]) ? W(d[e] * 10) - 5 : d[e] === "Z" ? "x" : d[e];
							}
							d = m.join(" ") || "x";
							f.path = d;
							if (P) {
			...
			if (this.added) {
				...
				if (c || d) {
					a.css({marginLeft:c, marginTop:d});
				}
				...
				if (b.tagName === "SPAN") {
					...
					var P = 0, T;
					v = ja(a.textWidth);
					var Y = a.xCorr || 0, H = a.yCorr || 0, U = [c, g, b.innerHTML, a.textWidth].join(",");
					if (U !== a.cTT) {
						if (K(c)) {
							j = c * ae;
							d = rb(j);
		...
		ma.prototype = Ca(ed.prototype, {Element:Db, isIE8:yc.indexOf("MSIE 8.0") > -1, init:function (a, b, c) {
			...
			return this.createElement("span").attr({text:a, x:W(b), y:W(c)}).css({whiteSpace:"nowrap", fontFamily:d.fontFamily, fontSize:d.fontSize});
		}, path:function (a) {
	...
	Oc.prototype = {init:function (a, b) {
		...
		if (!this.hasImportedEvents) {
			var a = Ca(this.series.options.point, this.options).events, b;
			this.events = a;
			for (b in a) {
				Qa(this, b, a[b]);
			}
		...
		if (!(a === this.state || this.selected && a !== "select" || c[a] && c[a].enabled === false || a && (f || e && !d.enabled))) {
			if (this.graphic) {
				...
				if (a) {
					if (!g) {
						b.stateMarkerGraphic = g = h.renderer.circle(0, 0, i[a].r).attr(i[a]).add(b.group);
					}
	...
	sb.prototype = {isCartesian:true, type:"line", pointClass:Oc, pointAttrToOptions:{stroke:"lineColor", "stroke-width":"lineWidth", fill:"fillColor", r:"radius"}, init:function (a, b) {
		...
		if (this.options.dataLabels.enabled) {
			var a, b, c = this.data, d = this.options, e = d.dataLabels, f, g = this.dataLabelsGroup, h = this.chart, i = h.renderer, j = h.inverted, m = this.type, v;
			v = d.stacking;
			var P = m === "column" || m === "bar", T = e.verticalAlign === null, Y = e.y === null;
			if (P) {
				if (v) {
					if (T) {
						e = Ca(e, {verticalAlign:"middle"});
					}
					if (Y) {
						e = Ca(e, {y:{top:14, middle:4, bottom:-6}[e.verticalAlign]});
					}
				} else {
					if (T) {
						e = Ca(e, {verticalAlign:"top"});
					}
				}
			}
			if (g) {
				...
				g = this.dataLabelsGroup = i.g("data-labels").attr({visibility:this.visible ? Ab : ob, zIndex:6}).translate(h.plotLeft, h.plotTop).add();
			}
			v = e.color;
			if (v === "auto") {
				v = null;
			...
			u(c, function (H) {
				...
				if (y) {
					...
					if (K(f)) {
						y = H.dataLabel = i.text(f, a, b).attr({align:C, rotation:e.rotation, zIndex:1}).css(e.style).add(g);
		...
		var a = this, b = a.options, c = a.graph, d = [], e, f = a.area, g = a.group, h = b.lineColor || a.color, i = b.lineWidth, j = b.dashStyle, m, v = a.chart.renderer, P = a.yAxis.getThreshold(b.threshold || 0), T = /^area/.test(a.type), Y = [], H = [];
		...
		if (T) {
			...
			if (f) {
				...
				a.area = a.chart.renderer.path(H).attr({fill:e}).add(g);
			}
		}
		if (c) {
			Hc(c);
		...
		if (!h) {
			h = a.clipRect = !b.hasRendered && b.clipRect ? b.clipRect : i.clipRect(0, 0, b.plotSizeX, b.plotSizeY);
			if (!b.clipRect) {
				b.clipRect = h;
			}
		}
		...
		if (g) {
			...
			a.tracker = e.renderer.path(c).attr({isTracker:true, stroke:ce, fill:jb, "stroke-width":b.lineWidth + 2 * f, visibility:a.visible ? Ab : ob, zIndex:b.zIndex || 1}).on(Kb ? "touchstart" : "mouseover", function () {
	...
	var hd = yb(sb, {type:"column", pointAttrToOptions:{stroke:"borderColor", "stroke-width":"borderWidth", fill:"color", r:"borderRadius"}, init:function () {
		...
		P = h * c.groupPadding;
		var T = (h - 2 * P) / f, Y = c.pointWidth, H = K(Y) ? (T - Y) / 2 : T * c.pointPadding, U = Ia(A(Y, T - 2 * H), 1), z = H + (P + ((g ? f - a.columnIndex : a.columnIndex) || 0) * T - h / 2) * (g ? -1 : 1), M = a.yAxis.getThreshold(c.threshold || 0), y = A(c.minPointLength, 5);
		u(v, function (C) {
			...
			C.shapeType = "rect";
			Z = sa(b.renderer.Element.prototype.crisp.apply({}, [e, Na, Ea, U, gb]), {r:c.borderRadius});
			if (e % 2) {
				Z.y -= 1;
				Z.height += 1;
			}
		...
		var a = this, b = a.chart, c = b.renderer, d, e, f = +new Date, g = a.options, h = g.cursor, i = h && {cursor:h}, j;
		u(a.data, function (m) {
			e = m.tracker;
			...
			if (m.y !== null) {
				if (e) {
					...
					m.tracker = c[m.shapeType](d).attr({isTracker:f, fill:ce, visibility:a.visible ? Ab : ob, zIndex:g.zIndex || 1}).on(Kb ? "touchstart" : "mouseover", function (v) {
	...
	wb.column = hd;
	ma = yb(hd, {type:"bar", init:function (a) {
	...
	ma = yb(sb, {type:"pie", isCartesian:false, pointClass:ma, pointAttrToOptions:{stroke:"borderColor", "stroke-width":"borderWidth", fill:"color"}, getColor:function () {
		...
		u(P, function (C) {
			...
			C.labelPos = [f[0] + z + rb(v) * y, f[1] + M + Cb(v) * y, f[0] + z + rb(v) * e, f[1] + M + Cb(v) * e, f[0] + z, f[1] + M, y < 0 ? "center" : v < T / 4 ? "left" : "right", v];
			C.percentage = Y * 100;
			C.total = a;
		});
		...
		u(this.data, function (i) {
			...
			if (d) {
				...
				i.graphic = b.arc(h).attr(sa(i.pointAttr[ib], {"stroke-linejoin":"round"})).add(i.group).shadow(f, g);
			}
			i.visible === false && i.setVisible(false);
		});
	}, drawDataLabels:function () {
		var a = this.data, b, c = this.chart, d = this.options.dataLabels, e = A(d.connectorPadding, 10), f = A(d.connectorWidth, 1), g, h, i = A(d.softConnector, true), j = d.distance, m = this.center, v = m[2] / 2;
		m = m[1];
		...
		if (d.enabled) {
			...
			for (a = T[0][0] && T[0][0].dataLabel && ja(T[0][0].dataLabel.styles.lineHeight); M--; ) {
				...
				for (y = 0; y < Na; y++) {
					...
					g.attr({visibility:U, align:h[6]})[g.moved ? "animate" : "attr"]({x:Y + d.x + ({left:e, right:-e}[h[6]] || 0), y:H + d.y});
					g.moved = true;
					if (P && f) {
						g = b.connector;
						...
						if (g) {
							...
							b.connector = g = this.chart.renderer.path(h).attr({"stroke-width":f, stroke:d.connectorColor || b.color || "#606060", visibility:U, zIndex:3}).translate(c.plotLeft, c.plotTop).add();
						}
					}
				}
			}
		}
	}, drawTracker:hd.prototype.drawTracker, getSymbol:function () {
	}});
	wb.pie = ma;
